...
> **Adelantos ya hechos (2026-06-21, durante Fase 5).** Validado con lint/build verdes:
> - ✅ **Paso 3 — `ImcCard` y `RiesgoCardiometabolico`** migrados a tokens semánticos
>   (`bg-success/10 text-success`, `bg-warning/10 text-warning`, `bg-destructive/10 text-destructive`),
>   sin `style` inline ni hex; `Badge` sin `text-black`. Resuelve los fondos rosados sobre el tema oscuro.
> - ✅ **Paso 5 (parcial) — `Date-picker.tsx`:** ya se le quitaron los colores hardcodeados.
> - ✅ **Paso 2 — Historial:** migrado a `PacienteWorkspaceHeader` + `Table` + `EmptyState` + `Pagination`,
>   con deltas y acciones por icono.
> - ⬜ Falta: páginas `nueva`/`editar`/`eliminar`, `MedicionForm`, `EvolucionDialog`
>   (Dialog + `recharts` con `--chart-*`).
...
> **Adelanto desde 5.5.** ✅ Lista de planes migrada al workspace compartido, cards consistentes y
> `EmptyState`. Quedan editor, detalle y vista de impresión para completar la fase.
